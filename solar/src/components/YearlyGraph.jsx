            ...
            (
              <React.Fragment key={`${index}-frag`} >
                <div className="verticalstrip" key={index}>
                  {dataPoint[1].includes("01T") && <div className="break flex flex-col-reverse" key={`${index}-break`} ><div className="pl-2  text-xs sm:text-xs md:text-base lg:text-base xl:text-base 2xl:text-base">{formatDateToGetMonthOnly(dataPoint[1])}</div></div>}
                </div>
